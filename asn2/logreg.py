...
def main():
  ...
  y_pred_train = X_test_bias@w >= 0
  kag = open("sub.txt", "w")
  kag.write("id,type\n")
  for res in range(len(y_pred_train)):
    kag.write(str(res))
    kag.write(",")
    if y_pred_train[res] == True:
      kag.write("1")
    else:
      kag.write("0")
    kag.write("\n")
...
def sigma(x, w):
  temp_w = x@w
  s = (1.0/(1.0+np.exp(-temp_w)))
  return s

def calculateNegativeLogLikelihood(X,y,w):
  p = np.sum(-y * np.log(sigma(X, w) + 0.0000000000001) + ((1 - y) * np.log(1-sigma(X, w) + 0.00000000001)))
  return p
...
def trainLogistic(X,y, max_iters=max_iters, step_size=step_size):
    ...
    for i in range(max_iters):
        ...
        # .
        nll = (sigma(X,w) - y) * X
        for j in range(w_grad.shape[0]):
          w_grad[j] = np.sum(nll[:,j])
